...
CRITICAL FIX: Generates main.csv with correct peripheral mappings (pX -> pX, NOT pX -> n1)
...
Creates:
  <repo_root>/models/patient_<pid>/
    - main.csv (FIXED peripheral mappings)
    - arterial.csv (patient-specific geometry)
    - p1.csv, p2.csv, ..., p47.csv (copied from template)
    - heart_kim_lit.csv (copied from template)
...
def find_patient_files(data_root: Path, pid: str) -> Tuple[str, Path, Path, Optional[Path]]:
    ...
    Supports multiple naming conventions:
      1. All named topcow_<modality>_<pid>.json in different folders
      2. nodes_<modality>_<pid>.json for nodes folder
    Preference: ct then mr
    """
    pid = str(pid).zfill(3)

    for modality in ["ct", "mr"]:
        # Try: all named topcow_X_025.json (most common)
        ...
        # Try: nodes folder has nodes_X_025.json instead
...
def find_terminal_nodes(arterial_df: pd.DataFrame) -> List[str]:
    """Find all peripheral terminal nodes (p1, p2, ..., p47) from arterial.csv"""
    starts = set(arterial_df['start_node'].dropna())
    ends = set(arterial_df['end_node'].dropna())
    
    terminals = ends - starts
    peripherals = sorted([n for n in terminals if str(n).startswith('p')])
...
def generate_main_csv(arterial_df: pd.DataFrame, output_path: Path, time_duration: float = 10.317):
    ...
    print(f"  Found {len(peripherals)} peripheral nodes for main.csv")
    ...
    for p in peripherals:
        num = p[1:]  # Extract number from 'p47' -> '47'
        main_node = f"N{num}p"
    ...
    print(f"  Generated main.csv with {len(peripherals)} peripherals correctly mapped")
...
def main():
    ...
    ap.add_argument("--template_model", default="Abel_ref2", help="template model folder")
    ap.add_argument("--force", action="store_true", help="overwrite output model if exists")
    args = ap.parse_args()
    ...
    print("=" * 78)
    print("PATIENT COW INJECTION (CORRECTED - Fixed peripheral mappings)")
    print("=" * 78)
    print(f"pid:            {pid}")
    ...
    print(f"Parsed {len(segs)} raw feature segments.")
    ...
    for src in template_dir.glob("*.csv"):
        ...
        copied_count += 1
    print(f"  Copied {copied_count} CSV files from template (parameter.csv, peripherals, heart, etc.)")
    ...
    print("\nGenerating main.csv with CORRECTED peripheral mappings:")
    ...
    print("=" * 78)
    print(f"Modified {len(modifications)} vessels in arterial.csv")
    print(f"\nCRITICAL FIX APPLIED:")
    print(f"  Peripherals now connect to their OWN terminal nodes (p1->p1, p47->p47)")
    print(f"  NOT all to n1 (which caused the -1.1e7 Pa crash)")
